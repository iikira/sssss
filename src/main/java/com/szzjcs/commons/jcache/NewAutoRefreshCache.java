...
import com.szzjcs.commons.jcache.monitor.Visitor;
...
public abstract class NewAutoRefreshCache<K, V> extends AutoRefreshCacheBase<K, V>
{
    ...
    // private static final Map<String, Long> reflushTimeMap = new
    // HashMap<String,
    // Long>();

    // protected synchronized static void collectReflushTime(String name, long
    // t)
    // {
    ...
    public NewAutoRefreshCache(Properties prop)
    {
        ...
        autoRecover = NewAutoRefreshCache.getPropertyBoolean(prop, "auto_recover", false);

        if (autoRecover)
        {
            long st = System.currentTimeMillis();
            recover(saveFilePath);
            NewAutoRefreshCache.logger.info(String.format("%s recovered within %d ms", saveFilePath,
    ...
    public String getReflushingStatus()
    {
        ...
        synchronized (futures)
        {
            ...
            else
            {
                if (!ft.isDone())
                {
                    NewAutoRefreshCache.logger.info(String.format("%s is reflushing, %d entries finished", getClass()
                            .getSimpleName(), size()));
    ...
    public synchronized String requestClear()
    {
        ...
        else
        {
            if (!ft.isDone())
            {
                NewAutoRefreshCache.logger.info(String.format("%s is reflushing", getClass().getSimpleName()));
                flag = false;
            }
            else
            {
        ...
        if (flag)
        {
            CacheFactory2.REFLUSH_EXECUTOR.submit(futures.get(getClass().getSimpleName()));
        }
        else
        {
            r = String.format("%s正在重刷中；之前已经有人开始刷了，但还需要一点儿时间刷完，稍候取结果吧:-)", getClass().getSimpleName());
        }
    ...
    private FutureTask<String> newReflushTask()
    {
        return new FutureTask<String>(new Callable<String>()
        {
            ...
            public String call() throws Exception
            {
                ...
                String s = String.format("%s reflushed. cost:%d ms", NewAutoRefreshCache.this.getClass()
                        .getSimpleName(), System.currentTimeMillis() - st);
                NewAutoRefreshCache.logger.info(s);
                return s;
            }
        });
    }
    ...
    private boolean recover(String file)
    {
        ...
        try
        {
            ...
            NewAutoRefreshCache.logger.info("recover cache from file: " + file);
        ...
        {
            NewAutoRefreshCache.logger.error(String.format("error recover file (%s), delete it", file), e);
            if (null != f)
            {
                f.delete();
            }
        ...
        finally
        {
            if (in != null)
            {
                ...
                {
                    NewAutoRefreshCache.logger.error("close error: " + file, e);
                }
            }
            if (ois != null)
            {
    ...
    private boolean save(String file)
    {
        ...
        NewAutoRefreshCache.logger.info("begin save cache to file: " + file);
        ...
        try
        {
            ...
            NewAutoRefreshCache.logger.info("cache size: " + entrySet.size() + "\tfile:" + file);
            ...
            while (it.hasNext())
            {
                ...
                {
                    NewAutoRefreshCache.logger.error("writeObject to file error: ", e);
                }
            }
            oos.flush();
        }
        ...
        {
            NewAutoRefreshCache.logger.error("save to file error: ", e);
            return false;
        }
        finally
        {
            if (outout != null)
            {
                ...
                {
                    NewAutoRefreshCache.logger.error("", e);
                }
            }
            if (oos != null)
            {
            ...
            NewAutoRefreshCache.logger.info("finish save cache to file: " + file);
        }
